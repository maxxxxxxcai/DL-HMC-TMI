...
#         df_res_temp['summary_path']=summary_path
        ...
        for j in range(1799):
            parameter13_3600=mCTdata_vicra_toolbox.RotTransMatrix_6Params(df_res_temp.at[j,'model parameters'], 2)
            df_res_temp.at[j+1,'model parameters'] = mCTdata_vicra_toolbox.DL_HMC_concat_VC_zty(parameter13_3600,df_res_temp.at[j+1,'model parameters 12']) 
...
def plot_vicra_network(tracer, patient, df_results, time_window, network_name, trans_range=[-10,10], rot_range=[-4,4]):
    ...
    df_results_patient=df_results_tracer.loc[df_results_tracer['PatientID'].str.contains(patient)]
    ...
    return
'''def plot_vicra_network(tracer, patient, df_results, time_window, network_name):
