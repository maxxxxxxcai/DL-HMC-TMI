...
import sys
sys.path.append(r'/data16/private/zc348/project/')
import DL_HMC_attention.mCT.util.python.mCTdata_vicra_toolbox as mCTdata_vicra_toolbox

from DL_HMC_attention.mCT.util.python.mCTdata_vicra_toolbox import RotTransMatrix_6Params,Relative_motion_A_to_B
...
def add_T_deltaT_12(df_train_tmp, df_val_tmp, df_test_tmp):
    ...
    df_train_tmp['T'] = df_train_tmp.apply(lambda row: 
                                     mCTdata_vicra_toolbox.Relative_motion_A_to_B_12(row['M_ref'], row['M_mov']),axis=1)
    df_val_tmp['T'] = df_val_tmp.apply(lambda row: 
                                     mCTdata_vicra_toolbox.Relative_motion_A_to_B_12(row['M_ref'], row['M_mov']),axis=1)
    df_test_tmp['T'] = df_test_tmp.apply(lambda row: 
                                     mCTdata_vicra_toolbox.Relative_motion_A_to_B_12(row['M_ref'], row['M_mov']),axis=1)
