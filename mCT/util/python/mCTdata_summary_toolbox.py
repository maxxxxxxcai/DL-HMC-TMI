...
def get_data(key, tracers):
    ...
    sheet=client.open('mCT_data_DL-HMC')

    # Get the different sheets corresponding to the different tracers
    tracers_sheet=['FPEB','EMO'] #must be updated when tracers are added 
...
def print_info(key, tracers):
    df, df_ready, patients_ready=get_data(key, tracers)
    ...
    for i in range(len(tracers)):
        ...
        patients_ready_int=(df_ready_int['UniqueID'].tolist())
...
#     data_type: the data type (real or simulated). Default: 'real'.
#     force_scan: a list of tuples (subjectID, injectionID) to force one scan to be used when multiple ones are available for the same patient. Default: None.
...
def compute_delta_T(tracer, patients, data_type='real', force_scan=None):
    ...
    df, df_ready, patients_ready=get_data('', tracer)
    df_ready= df_ready.loc[df_ready['UniqueID'].isin(patients)].reset_index(drop=True) 
    
    # Transform the dates (e.g. 2015/7/3 => 20150703)
    dates=df_ready[['date', 'tracer']]
...
#     img_power=df_ready['image_power'].tolist()
    ...
    patients_ready=(df_ready[['PatientID', 'tracer']])
    patients=list(patients_ready['PatientID'])
    analysis_id=df_ready[['InjectionID', 'tracer']]  
    analysis_id_only=list(analysis_id['InjectionID'])
    uniqueIDs=list(df_ready['UniqueID'])
    
    if data_type=='real':
        for k in range(len(df_ready)):
    ...
    if data_type=='simulation':
        for k in range(len(df_ready)):
            paths.append(r'/data16/public/registration-brain_PET/data/simulation/'+str(tracer1)+'/'+str(dates_nb[k])+'_'+str(patients[k])+'/'+str(analysis_id_only[k]))
    ...
    if data_type=='real':
        for l in range(len(patients)):
            ...
            # summaries_paths.append(os.path.join(paths[l],'Summary_'+patients[l]+'_1800_5400.csv'))
    ...
    def Relative_motion_A_to_B_12(img_vc1, img_vc2):
        ...
        R = mCTdata_vicra_toolbox.Relative_motion_A_to_B(A, B)
        return R[1:13]
    ...
    for path in summaries_paths:
        ...
        summary['UniqueID']=uniqueIDs[j]
        summary['T'] = summary['MATRIX'].apply(lambda x: mCTdata_vicra_toolbox.RotTransMatrix_6Params(x, 1))
        summary['delta_T'] = summary['T'].apply(lambda t: delta_T_magnitude(summary.loc[0,'T'], t))
        delta_T_all_temp.append(summary['delta_T'])
        summary['relative_T'] = summary['MATRIX'].apply(lambda t: mCTdata_vicra_toolbox.RotTransMatrix_6Params(Relative_motion_A_to_B_12(summary.loc[0,'MATRIX'], t), 1)) #would be wrt the first s ?
...
def data_analysis_display(tracers, patients):
    ...
    for tracer in tracers:
        ...
        df, df_ready, patients_ready=get_data('', tracers[l])
...
def make_gif_3Dcloud(tracer, patients): 
    ...
    for patient in patients:
        ...
        for i in range(num_display):
            ...
            plt.title(str(patient)+' n='+str(i*20))
        ...
        img_gif=images[0]
        img_gif.save('cloud_gif'+str(patient)+'.gif', format="GIF", append_images=images,
