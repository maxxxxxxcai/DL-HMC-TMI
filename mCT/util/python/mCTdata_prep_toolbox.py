...
import mCTdata_vicra_toolbox as mCTdata_vicra_toolbox
...
def clean_df_cloudnorm(df):
    cols_to_keep=['PatientID','InjectionID','PatientType','ScanStart',
                   'ThreeD_Cloud', 'COD_nomask','M', 'T','image_power']
...
def clean_df(df):
    cols_to_keep=['PatientID','InjectionID','UniqueID','PatientType','ScanStart',
...
#         print(ref_index)
        ...
        df_temp['T'] = df_temp.apply(lambda row: 
                                mCTdata_vicra_toolbox.RotTransMatrix_6Params(
                                    mCTdata_vicra_toolbox.Relative_motion_A_to_B_12(row['M_x'], row['M_y']), 1), axis=1
                                )
        if (i == 0):
            df_final = df_temp
        else:
...
#     df = pd.read_csv(dir)
    ...
    # df_m = pd.DataFrame()
    df['MATRIX'] = M
    df['T'] = df['MATRIX'].apply(lambda x: mCTdata_vicra_toolbox.RotTransMatrix_6Params(x, 1))
    df['delta_T'] = df['T'].apply(lambda t: delta_T_magnitude(df.loc[0,'T'], t))
    df['relative_T'] = df['MATRIX'].apply(lambda t: mCTdata_vicra_toolbox.RotTransMatrix_6Params(Relative_motion_A_to_B_12(df.loc[0,'MATRIX'], t), 1))
