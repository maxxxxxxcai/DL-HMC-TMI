...
import glob
import sys
sys.path.append(sys.path[0]+'/util/python')
...
def print_info(tracers):
    df, df_ready, patients_ready=get_data('', tracers)
...
#     img_power=df_ready['image_power'].tolist()
    ...
    if data_type=='real':
        for k in range(len(df_ready_clean)):
            paths.append(r'/data16/public/registration-brain_PET/data/real/'+str(tracer[0])+'/'+str(dates_nb[k])+'_'+str(patients[k])+'/'+str(analysis_id_only[k]))
    if data_type=='simulation':
        for k in range(len(df_ready_clean)):
