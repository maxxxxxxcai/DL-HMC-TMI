...
def data_split_sample(df, train_frac=1.0, val_frac=0.0, test_frac=0.0, ref_suffix="_ref", mov_suffix="_mov", force_first_frame=False, distr=None):
    ...
    distr: delta_t distribution. Valid values are None, 'bell', 'uniform'. Default:None. 
    ...
    # Create the probability sampling map
    P = np.ones((n,n))
    P = np.triu(P)

    if distr=='bell':
        for i in range(n):
            P[np.triu_indices_from(P, k=i)] = i+1
    elif distr=='uniform':
        for i in range(n):
            P[np.triu_indices_from(P, k=i)] = 1/(n-i)
    # Remove any validation and test set indices
    P[:,I_val] = 0
    P[I_val,:] = 0
    P[:,I_test] = 0
    P[I_test,:] = 0
   # Normalize the probs
    ...
    I_train = np.sort(np.random.choice(M.flatten(), size=int(n*train_frac), replace=False, p=P.flatten()))
...
def data_continuous_sample(df,train_frac=1.0,val_frac=0.0, test_frac=0.0, ref_suffix="_ref", mov_suffix="_mov", force_first_frame=False):
